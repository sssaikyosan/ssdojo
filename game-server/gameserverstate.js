...
export class GameServerState {
    ...
    matchMaking() {
        ...
        if (matchMakingPlayers.length >= 2) {
            ...
            while (matchMakingPlayers.length >= 2) {
                ...
                // 両プレイヤーにマッチング完了を通知
                // player_id の取得方法が不明確だが、ratingProcess で使われている前提なのでそのままにする
                const rate1 = this.ratings[this.players[player1].player_id];
                const rate2 = this.ratings[this.players[player2].player_id];

                const player1rating = getDisplayRating(rate1.rating, rate1.total_games);
                const player2rating = getDisplayRating(rate2.rating, rate2.total_games);
    ...
    getUserRating(player_id) {
        ...
        return getDisplayRating(this.ratings[player_id].rating, this.ratings[player_id].games)
    }

    getUserGames(player_id) {
        if (!this.ratings[player_id]) return null;
        return this.ratings[player_id].games;
    }
